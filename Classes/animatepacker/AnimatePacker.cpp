...
CCAnimation* AnimatePacker::getAnimation(const char *name )
{
...
CCSequence *AnimatePacker::createSequence(CCArray *actions)
{
	...
	CCSequence *seq = CCSequence::create((CCFiniteTimeAction*)actions->objectAtIndex(0),
		(CCFiniteTimeAction*)actions->objectAtIndex(1), NULL);
	for (size_t i = 2; i < actions->count(); ++i) {
		seq = CCSequence::create(seq, (CCFiniteTimeAction*)actions->objectAtIndex(i), NULL);
	}
	return seq;
}
...
CCSequence *AnimatePacker::createSequence(CCFiniteTimeAction *pAction1, CCFiniteTimeAction *pAction2, ...)
{
	...
	CCSequence *pPrev = CCSequence::create(pAction1, pAction2, NULL);
