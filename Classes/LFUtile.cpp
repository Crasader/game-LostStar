...
{//只支持b2PolygonShape和b2CircleShape
	...
	{
		CCAssert(false,"只支持b2PolygonShape和b2CircleShape");
	}
...
void cutoutText(CCRenderTexture* render, CCNode* bg, CCLabelTTF* label)
{
    if(render == NULL || bg == NULL || label == NULL)
    {
        return;
    }
	
	CCSize renderSize = render->getSprite()->getContentSize();
	CCPoint renderPosWorld = render->getParent()->convertToWorldSpace(render->getPosition());

	render->begin();

	//bg
	CCPoint bgPos = bg->getPosition();
	CCPoint bgPosWorld = bgPos;
	if(bg->getParent() != NULL)
	{
		bgPosWorld = bg->getParent()->convertToWorldSpace(bgPos);
	}
	bg->setPosition(ccp(renderSize.width/2, renderSize.height/2) + bgPosWorld - renderPosWorld);
	bg->visit();
	bg->setPosition(bgPos);

	//label
	CCPoint labelPos = label->getPosition();
	CCPoint labelPosWorld = label->getParent()->convertToWorldSpace(labelPos);
	label->setPosition(ccp(renderSize.width/2, renderSize.height/2) + labelPosWorld - renderPosWorld);
	label->setColor(ccc3(255, 255, 255));
	label->setVisible(true);
	ccBlendFunc blend = {GL_ZERO, GL_SRC_COLOR};
	...
	//render->newCCImage()->saveToFile("xxx.png", false);
}
