...
void* CCTexture2DMutable::keepData(void* data, size_t lenght)
{
...
bool CCTexture2DMutable::initWithData(const void* data, CCTexture2DPixelFormat pixelFormat, size_t width, size_t height, const CCSize& size)
{
    ...
    data_ = (void*) data;
#if CC_MUTABLE_TEXTURE_SAVE_ORIGINAL_DATA
    size_t max = width*height*bytesPerPixel_;
...
void CCTexture2DMutable::apply()
{
...
#if CL_TEXTURE_MEMORY_USAGE_ENABLED
	size_t bitsPerPixel;
	//Hack: bitsPerPixelForFormat returns wrong number for RGB_888 textures. See function.
	if(m_ePixelFormat == kCCTexture2DPixelFormat_RGB888)
	{
		bitsPerPixel = 24;
	}
	else
	...
	size_t bytes = m_uPixelsWide * m_uPixelsHigh * bitsPerPixel / 8;
	CCDirector::sharedDirector()->reportTextureAlloc(bytes);
	m_uTextureSize = bytes;
#endif
