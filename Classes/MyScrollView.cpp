...
float maxMoveForClick = 50.0f;//移动多少像素之内属于点击;
float minEffectiveDisForClick = 50.0f;
...
void MyScrollView::ccTouchEnded(CCTouch *pTouch, CCEvent *pEvent)
{
	...
	// 不让他自己滑动
	// 就是为了unschedule(schedule_selector(CCScrollView::deaccelerateScrolling));
...
void MyScrollView::srollToPage(int pageIndex,float pageSizeX,float pageInval_X,float duration)
{
	...
	int selectIndex = mSelectedIndex;
