...
{
	// 一个纯透明的背景，作为真正的容器。
	//return CCSprite::create(ResManager::getManager()->getSharedFilePath("select_bg.png").c_str());
...
CCArray* GridScrollView::getPageItems(int pageIndex)
{
	...
	{
		if (i > (int)gridItemArray->capacity()-1)
			break;
...
void GridScrollView::setPageItemOpacity(int pageIndex,GLubyte opacity)
{
	...
	for (int i=start;i<end;i++)
	{
		if ((size_t)i > gridItemArray->capacity() -1 )
			break;
