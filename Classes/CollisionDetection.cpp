...
bool CollisionDetection::areTheSpritesColliding(cocos2d::CCSprite*spr1,cocos2d::CCSprite*spr2,bool pp,CCRenderTexture*_rt){
	...
	if(r1.intersectsRect(r2)){
		...
		size_t x=intersection.origin.x;
		size_t y=intersection.origin.y;
		size_t w=intersection.size.width;
		size_t h=intersection.size.height;
		...
		ccBlendFunc fc = {CC_BLEND_SRC,CC_BLEND_DST};
		spr1->setBlendFunc(fc);
		...
		// Read buffer
		size_t step=1;
		for(size_t i=0;i<numPixels;i+=step){
