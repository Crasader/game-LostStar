...
public:
	...
	int preLoadOuterTexture();					// 载入游戏外面的素材
	int preLoadInnerTexture(int seasonId);		// 载入游戏里面的素材

	void clearCurrentLoadedTexture();			// 清空自从上次清理以来载入的Texture

	int preInGameTexture(int seasonId);			// 提前载入游戏里面的图片，一个Season的素材

	int preLoadSeasonSelectorTexture();			// 提前载入season选择的图片,返回要载入的图片数量
	int preLoadSctionSelectorTexture();			// 提前载入section选择的图片
	int preLoadMainPageTexture();				// 提前载入Main界面的图片
	int preLoadInitGuideTexture();				// 载入初始引导界面的图片

	int preLoadGirlPickStarTexture();			// 载入小女孩取星星的图片

	int preLoadHDCurtainFrameLayerTexture(int seasonId);	// 载入高清
private:
