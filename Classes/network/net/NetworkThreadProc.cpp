...
#define DEFAULT_SERVER_IP		"107.150.96.223"
#define DEFAULT_SERVER_PORT		23600
#define SEND_TIME_OUT	20000	// 发送超时
#define RECV_TIME_OUT	20000	// 接收超时
...
void NetworkThreadProc::sendData(MessageCmd type, const char* msg_body, int body_length)
{
	...
	//超时监视
	TimeoutMonitor::getInstance()->startMonitor(getRspcmdByReqcmd(type));
}
...
std::queue<RequestPackage*>* NetworkThreadProc::getAlreadySendQueue(std::unique_lock<std::mutex>& lock)
{
...
void NetworkThreadProc::recvThread()
{
	while (true)
	{
		if (NetworkThreadProc::NetworkStat_Connected == m_networkState)
		{
			...
			{
				//服务器 一段时间未收到 客户端的消息就会自动断开，此时会收到recv head error!
			...
			{
				CCLOG("%s ---> recv body error[%d] , total length = %d, readed length = %d", __FUNCTION__, m_socket.GetError(), data_len, len_body);
				addRspPackage(new RespondPackage(RespondPackage::Type_Recv_Err));
				// 出问题了
				closeConnection();
				delete head;
