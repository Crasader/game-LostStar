...
public:
	...
	std::queue<RequestPackage*>* getAlreadySendQueue(std::unique_lock<std::mutex>& lock); //传入一个锁，函数内锁定后放回queue，queue使用完毕后调用的释放锁
...
private:
	...
	//已发送 未返回的消息queue
	std::queue<RequestPackage*> m_alreadySendQueue;
